...
export default function PetPosting(props) {
  ...
  useEffect(() => {
    if (date) {
      ...
      const filtered = allPosts.filter((post) => {
        return (
          post.name.trim().toLowerCase().includes(normalSearch) ||
          post._id.trim().toLowerCase().includes(normalSearch)
        );
      });
      setFilteredPosts(filtered);
      ...
      const filtered = allPosts.filter((post) => {
        return (
          post.postalCode.toLowerCase().includes(locationSearch) ||
          post.location.toLowerCase().includes(locationSearch)
        );
      });
      setFilteredPosts(filtered);
    }
    ...
    <>
      <div className="posting" style={{ minHeight: "100vh" }}>
        ...
        <Container className="filter-components">
            ...
            <div className="d-flex justify-content-center combined-search mt-5">
              <div>
                <RegularFilter
                  handleRegularChange={handleRegularChange}
            ...
            <Col className="d-flex justify-content-center single-search mt-2">
              <div>
              <LocationFilter
